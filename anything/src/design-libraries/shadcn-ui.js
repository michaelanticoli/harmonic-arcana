...
function CustomAccordion({ data, type, className, }) {
    return (<Accordion type={type} collapsible className={className}>
      {data.map((item) => (<AccordionItem key={item.value} value={item.value}>
...
function CustomAlertDialog({ triggerLabel, title, description, cancelLabel = 'Cancel', actionLabel, }) {
    return (<AlertDialog>
        ...
        <BreadcrumbItem>
          <DropdownMenu>
            ...
            <DropdownMenuContent align="start">
              {dropdownMenuItems.map((item, index) => (<DropdownMenuItem key={index} onClick={item.onClick}>
                  {item.label}
                </DropdownMenuItem>))}
            </DropdownMenuContent>
          </DropdownMenu>
...
function CustomCarousel({ items }) {
    return (<Carousel opts={{
      ...
      <CarouselContent>
        {items.map((item, index) => (<CarouselItem className="md:basis-1/2 lg:basis-1/3" key={index}>
            {item}
          </CarouselItem>))}
      </CarouselContent>
      <CarouselPrevious />
...
function CollapsibleSection({ nonCollapsedItems, collapsedItems, title, }) {
...
function CommandPalette({ commandGroups, asDialog = false, dialogTriggerKey = 'k', }) {
    ...
    const render = (<Command className="rounded-lg border shadow-md">
      <CommandInput placeholder="Type a command or search..."/>
      <CommandList>
        {commandGroups.length === 0 && (<CommandEmpty>No results found.</CommandEmpty>)}
        {commandGroups.map((group) => (<Fragment key={group.heading}>
            <CommandGroup heading={group.heading}>
              {group.items.map((item) => (<CommandItem onSelect={item.onSelect} key={item.label}>
...
function CustomContextMenu({ items, triggerLabel, }) {
    const renderMenuItems = (menuItems) => menuItems.map((item) => (<Fragment key={item.label}>
        {item.subItems ? (<ContextMenuSub key={item.label}>
          ...
          </ContextMenuSub>) : (<ContextMenuItem onSelect={item.onSelect} key={item.label} disabled={item.disabled}>
            {item.label}
...
function CustomDialog({ triggerButtonText, title, description, children, open, onOpenChange, footer, }) {
...
function CustomDrawer({ title, description, openButtonText = 'Open', footer, children, }) {
    return (<Drawer>
...
function CustomDropdown({ triggerLabel, menuLabel, items, }) {
    const renderMenuItems = (itemsToRender) => itemsToRender.map((item, index) => (<Fragment key={index}>
        {item.subItems ? (<DropdownMenuSub>
          ...
          </DropdownMenuSub>) : (<DropdownMenuItem>
            {item.content}
            <span>{item.label}</span>
...
function CustomMenubar({ menuData, }) {
    return (<Menubar>
      {menuData.map((menu, menuIndex) => (<MenubarMenu key={menu.trigger}>
          <MenubarTrigger>{menu.trigger}</MenubarTrigger>
          <MenubarContent>
            {menu.items.map((item, itemIndex) => {
                switch (item.type) {
                    case 'separator':
                        return <MenubarSeparator key={`${menuIndex}-${itemIndex}`}/>;
                    case 'submenu':
                        return (<MenubarSub key={item.label}>
                      <MenubarSubTrigger>{item.label}</MenubarSubTrigger>
                      <MenubarSubContent>
                        {item.items.map((subItem, subItemIndex) => subItem.type === 'item' && (<MenubarItem onSelect={subItem.onSelect} key={`${menuIndex}-${itemIndex}-${subItemIndex}`} disabled={subItem.disabled}>
                                {subItem.label}
                                {subItem.shortcut && (<MenubarShortcut>
                                    {subItem.shortcut}
                                  </MenubarShortcut>)}
                              </MenubarItem>))}
                      </MenubarSubContent>
                    ...
                    case 'item':
                        return (<MenubarItem key={item.label} disabled={item.disabled} onSelect={item.onSelect}>
                      {item.label}
...
function CustomNavigation({ sections, }) {
    return (<NavigationMenu>
      <NavigationMenuList>
        {sections.map((section, sectionIndex) => (<NavigationMenuItem key={sectionIndex}>
            <NavigationMenuTrigger>{section.trigger}</NavigationMenuTrigger>
            <NavigationMenuContent>
              <ul className="grid gap-3 p-6 md:w-[400px] lg:w-[500px] lg:grid-cols-[.75fr_1fr]">
                {section.items.map((item, itemIndex) => (<li key={itemIndex} className={`row-span-${item.rowSpan ?? 1}`}>
                    <NavigationMenuLink asChild>
                      <a className="block select-none space-y-1 rounded-md p-3 leading-none no-underline outline-none transition-colors hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground" href={item.href}>
                        {item.logo}
...
function CustomPagination({ currentPage, totalPages, onPageChange, }) {
    ...
    return (<Pagination>
      <PaginationContent>
        <PaginationItem>
          <PaginationPrevious size={'default'} href="#" onClick={(e) => {
            e.preventDefault();
        ...
        {startPage > 1 && (<>
            <PaginationItem>
              <PaginationLink size={'default'} href="#" onClick={(e) => {
                e.preventDefault();
                onPageChange(1);
            }}>
                1
        ...
        {pageNumbers.map((page) => (<PaginationItem key={page}>
            {page === currentPage ? (<PaginationLink size={'default'} href="#" isActive>
                {page}
              </PaginationLink>) : (<PaginationLink size={'default'} href="#" onClick={(e) => {
                    e.preventDefault();
                    onPageChange(page);
                }}>
                {page}
        ...
        <PaginationItem>
          <PaginationNext size={'default'} href="#" onClick={(e) => {
            e.preventDefault();
...
function CustomSelect({ placeholder, value, onValueChange, groups, }) {
      ...
      <SelectContent>
        ...
        {groups.map((group, index) => (<Fragment key={index}>
            <SelectGroup>
              ...
              {group.items.map((item) => (<SelectItem key={item.value} value={item.value} disabled={item.disabled}>
                  {item.label}
                </SelectItem>))}
            </SelectGroup>
...
function CustomTooltip({ children, content, defaultOpen, open, onOpenChange, delayDuration, }) {
    return (<TooltipProvider>
      <Tooltip defaultOpen={defaultOpen} open={open} onOpenChange={onOpenChange} delayDuration={delayDuration}>
